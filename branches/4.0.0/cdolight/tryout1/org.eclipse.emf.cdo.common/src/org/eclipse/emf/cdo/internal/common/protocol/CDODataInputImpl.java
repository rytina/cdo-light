...
public abstract class CDODataInputImpl extends ExtendedDataInput.Delegating implements CDODataInput
{
  ...
  {
    int size1 = readInt();
    List<CDORevision> newObjects = new ArrayList<CDORevision>(size1);
    for (int i = 0; i < size1; i++)
    {
      CDORevision data =  readCDORevision();
      newObjects.add(data);
    }

    int size2 = readInt();
    List<CDORevisionDelta> changedObjects = new ArrayList<CDORevisionDelta>(size2);
    for (int i = 0; i < size2; i++)
    {
      CDORevisionDelta data = readCDORevisionDelta();
      changedObjects.add(data);
    }

    int size3 = readInt();
    List<Long> detachedObjects = new ArrayList<Long>(size3);
  ...
  private long readCDOIDObject(int subTypeOrdinal) throws IOException
  {
    return readLong();
  }
